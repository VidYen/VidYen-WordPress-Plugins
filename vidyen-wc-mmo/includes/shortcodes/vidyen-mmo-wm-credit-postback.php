...
function vidyen_mmo_wm_credit_postback_func($atts)
{
...
*/
	...
	if  (isset($_POST['userid']) AND isset($_POST['points']))
	{
		if (isset($_POST['userid']))
		{
			$game_id = sanitize_text_field($_POST['userid']);
		}
		...
		$vyps_meta_id = 'mmo'  . $game_id . $transactionId_sanitized; //the meta_id will be adgate with userid plus the transaction id. To see if its unique.
		...
		{
				return vidyen_mmo_wm_point_credit_func( $point_id, $point_amount, $game_id, $reason, $vyps_meta_id ); //I knew I had a good reason to use this
...
add_shortcode( 'vidyen-mmo-wm-credit', 'vidyen_mmo_wm_credit_postback_func');
