...
function vidyen_mmo_wm_point_credit_func( $point_id, $point_amount, $game_id, $reason, $vyps_meta_id )
{
	//$WPDB calls
	global $wpdb;
	...
	$point_amount = abs(intval($point_amount)); //Realized that to credit I need to subtract
	$game_id = sanitize_text_field($game_id);
	$reason = sanitize_text_field($reason);
	$vyps_meta_id = sanitize_text_field($vyps_meta_id);
	$user_id = 0;

	//credition for first point id
	$data = [
			'point_id' => $point_id,
