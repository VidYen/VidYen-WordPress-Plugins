...
/*** POINT ICON FUNCTION ***/
function vidyen_rts_icon_func($point_id)
{
  ...
  //$icon_html = '<img src="'.$sourceIcon.'" width="32" height="32" title="'.$sourceName.'">';
  $icon_html = '<img src="'.$sourceIcon.'" title="'.$sourceName.'" style="height:32px;">';

  return $icon_html;
}

function vidyen_rts_unit_icon_func($point_id)
{
  //The usual suspects to get the sql calls up
  global $wpdb;
	$table_name_points = $wpdb->prefix . 'vyps_points';
  ...
  //It's not like I couldn't just return the above and not the below in its own function later
  //$icon_html = '<img src="'.$sourceIcon.'" width="32" height="32" title="'.$sourceName.'">';
  $icon_html = '<img src="'.$sourceIcon.'" title="'.$sourceName.'" style="height:64px;">';

  return $icon_html;
}

function vidyen_rts_building_icon_func($point_id)
{
  //The usual suspects to get the sql calls up
  global $wpdb;
	$table_name_points = $wpdb->prefix . 'vyps_points';
  ...
  //It's not like I couldn't just return the above and not the below in its own function later
  //$icon_html = '<img src="'.$sourceIcon.'" width="32" height="32" title="'.$sourceName.'">';
  $icon_html = '<img src="'.$sourceIcon.'" title="'.$sourceName.'" style="height:256px;">';
