...
function vyps_adscend_redeem_func( $atts )
{
	...
	*/
	...
	if (function_exists('adscend_pro'))
	{
		$VYPS_power_row = ''; //No branding if procheck is correct.
	}
  ...
  /*** MoneroOcean Gets***/
  //Site get
  $site_url = "https://adscendmedia.com/adwall/api/publisher/{$pub_id}/profile/{$adwall_id}/user/{$sub_id}/transactions.json";
  $site_adscend_response = wp_remote_get( $site_url );
  if ( is_array( $site_adscend_response ) )
  {
    $site_adscend_response = $site_adscend_response['body']; // use the content
    $site_adscend_response = json_decode($site_adscend_response, TRUE);
    if (array_key_exists('currency_count', $site_adscend_response))
    {
			$balance = floatval($site_adscend_response['currency_count']);
    }
    else
    {
      $balance = 0;
  ...
  $balance = intval($balance);
	...
	*/
	...
	$response = wp_remote_post( $url, array(
		...
		'httpversion' => '1.0',
		'blocking' => true,
		'body' => array('api_key' => $api_key, 'currency_adjustment' => $adj_balance)
	  )
	);
