...
add_shortcode( 'vyps-as-watch', 'vyps_wannads_func');


//The redeem will need some cleansing but not much. BTW I trust wannads less than I do Coinhive. -Felty
function vyps_wannads_redeem_func( $atts ) {
	...
	*  But Wannads was being a pain. Oh. The API key is on
	...
	*  It's a shame Wannads didn't copy Coin Hive.
	*/
	...
	if (isset($_POST["redeem"])){
		...
		return "<form method=\"post\">
                ...
                <input type=\"submit\" class=\"button-secondary\" value=\"Redeem Wannads\" onclick=\"return confirm('You are about to sync your Wannads point with this site. Are you sure?');\" />
                </form>
								$VYPS_power_row";
	}
	...
	*/
	...
	$url = "https://wannadsmedia.com/adwall/api/publisher/{$pub_id}/profile/{$adwall_id}/user/{$sub_id}/transactions.json";
  ...
  //Here goes the cleansing. In theory one could have a really large point system on the Wannads side, but you really shouldn't.
  $balance = intval($balance);
	...
	$url = "https://wannadsmedia.com/adwall/api/publisher/{$pub_id}/profile/{$adwall_id}/user/{$sub_id}/transactions.json";
	//$url = "https://wannadsmedia.com/adwall/api/publisher/{$pub_id}/profile/{$adwall_id}/user/{$sub_id}/transactions.json?api_key={$api_key}&currency_adjustment={$adj_balance}";
	...
	if( $balance > 0 )
		{
			...
			$table_name_log = $wpdb->prefix . 'vyps_points_log';
			$reason = "Wannads"; //Felty's NOTE: I feel this could be user set, but there could be issues letting the admins have that much power.
	...
	*/
	return "<b>" . $amount . " points redeemed.</b><br><br><form method=\"post\">
                <input type=\"hidden\" value=\"\" name=\"redeem\"/>
                <input type=\"submit\" class=\"button-secondary\" value=\"Redeem Wannads\" onclick=\"return confirm('You are about to synce your Wannads point with this site. Are you sure?');\" />
                </form>
								$VYPS_power_row";

}

add_shortcode( 'vyps-as-redeem', 'vyps_wannads_redeem_func');
